...
export function createWeapons(deps) {
  const { camera } = deps;
  const rules = globalThis.MultiplayerRules;
  if (!rules || !Array.isArray(rules.WEAPONS)) throw new Error('MultiplayerRules não foi carregado.');
...
const builders = [buildRifle, buildShotgun, buildDMR, buildBazooka, buildPlasma, buildKnife, buildDMR, buildShotgun];
const arsenal = rules.WEAPONS.map((definition, index) => makeWeapon(definition, builders[index]));
